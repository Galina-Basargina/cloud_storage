-- Обновляет базу данных с v1 до v2

insert into coursework.users values (0, 'admin', ':nologin:');

alter table coursework.auth add logged_at timestamp not null default current_timestamp;

alter table coursework.folders drop constraint folders_owner_fkey;
alter table coursework.folders add constraint folders_owner_fkey foreign key (owner) references coursework.users(id) on delete cascade;

alter table coursework.shares_folders rename to shared_folders;
alter table coursework.shared_folders rename column read_onle to read_only;
alter table coursework.shared_folders add constraint shared_folders_unique unique (folder,recipient);

alter table coursework.shares_files rename to shared_files;
alter table coursework.shared_files rename column read_onle to read_only;
alter table coursework.shared_files add constraint shared_files_unique unique (file,recipient);
